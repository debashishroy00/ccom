...
def handle_traditional_commands(args, orchestrator):
    ...
    # Handle SDK integration commands (v5.0)
    elif args.sdk_status:
        handle_sdk_status_command(orchestrator)
        return True
    elif args.migration_recommendations:
        handle_migration_recommendations_command(orchestrator)
        return True
    elif args.set_agent_mode:
        handle_set_agent_mode_command(orchestrator, args.set_agent_mode)
        return True
    elif args.migrate_to_sdk:
        handle_migrate_to_sdk_command(orchestrator)
        return True
...
# ===== SDK INTEGRATION COMMAND HANDLERS (v5.0) =====

def handle_sdk_status_command(orchestrator):
    """Handle SDK status display command"""
    try:
        orchestrator.show_sdk_status()
    except Exception as e:
        print(f"❌ Error displaying SDK status: {e}")


def handle_migration_recommendations_command(orchestrator):
    """Handle migration recommendations command"""
    try:
        print("🤖 **CCOM MIGRATION RECOMMENDATIONS**")
        print("=" * 60)

        recommendations = orchestrator.get_migration_recommendations()

        print(f"📊 **Current Configuration**:")
        print(f"  Mode: {recommendations['current_mode'].upper()}")
        print(f"  SDK Agents Available: {recommendations['sdk_agents_available']}/{recommendations['total_agents']}")
        ...
        if recommendations['recommendations']:
            ...
            print(f"\n✅ **No recommendations** - System optimally configured")

        print(f"\n📈 **Performance Comparison**:")
        perf = recommendations['performance_comparison']
        print(f"  SDK Success Rate: {perf['sdk_success_rate']}")
        print(f"  Legacy Success Rate: {perf['legacy_success_rate']}")
        print(f"  SDK Average Time: {perf['sdk_avg_time']}")
        print(f"  Legacy Average Time: {perf['legacy_avg_time']}")

        print("=" * 60)

    except Exception as e:
        print(f"❌ Error getting migration recommendations: {e}")


def handle_set_agent_mode_command(orchestrator, mode):
    """Handle set agent mode command"""
    try:
        print(f"🔧 **SETTING AGENT MODE** to {mode.upper()}...")

        success = orchestrator.set_agent_mode(mode)

        if success:
            print(f"✅ Agent mode successfully set to: {mode}")
            print("💡 Use 'ccom --sdk-status' to verify the change")
        else:
            print(f"❌ Failed to set agent mode to: {mode}")

    except Exception as e:
        print(f"❌ Error setting agent mode: {e}")


def handle_migrate_to_sdk_command(orchestrator):
    """Handle migrate to SDK command"""
    try:
        import asyncio

        print("🚀 **CCOM SDK MIGRATION** – Checking migration readiness...")

        # Run migration asynchronously
        success = asyncio.run(orchestrator.migrate_to_sdk_mode())

        if success:
            print("\n🎉 **MIGRATION SUCCESSFUL!**")
            print("💡 Your CCOM installation now uses modern SDK-based agents")
            print("💡 Run 'ccom --sdk-status' to see the new configuration")
        else:
            print("\n⚠️ **MIGRATION NOT READY**")
            print("💡 Run 'ccom --migration-recommendations' for guidance")

    except Exception as e:
        print(f"❌ Error during SDK migration: {e}")
        print("💡 Try 'ccom --migration-recommendations' for troubleshooting")
...
def show_help():
    ...
    print(
        """
🚀 CCOM v5.0 - Claude Code Orchestrator and Memory with SDK Integration
...
SDK INTEGRATION (NEW in v5.0):
  ccom --sdk-status                 → Show SDK integration status and metrics
  ccom --migration-recommendations  → Get migration guidance
  ccom --set-agent-mode hybrid      → Set agent mode (sdk/markdown/hybrid)
  ccom --migrate-to-sdk             → Migrate to full SDK mode
  ccom --force-sdk                  → Force SDK agents for this command
  ccom --streaming                  → Enable real-time streaming output
...
EXAMPLES:
  ...
  ccom quality --streaming          → Quality check with real-time feedback
  ccom --status                     → Traditional status check

ENTERPRISE FEATURES:
  ✅ Claude Code native integration (prefix-based activation)
  ✅ Modern SDK-based agents with streaming support
  ✅ Intelligent agent routing (SDK/legacy hybrid)
  ✅ Performance monitoring and migration tools
...
MIGRATION PATH:
  1. Start: ccom --sdk-status
  2. Test:  ccom quality --force-sdk
  3. Check: ccom --migration-recommendations
  4. Migrate: ccom --migrate-to-sdk

For more info: https://github.com/debashishroy00/ccom
For migration help: See MIGRATION_GUIDE_v5.md
"""
    )
