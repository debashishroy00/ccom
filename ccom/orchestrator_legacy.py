#!/usr/bin/env python3
"""
CCOM Orchestrator v5.0 - Claude Code Integration Layer with SDK Support
Provides orchestration capabilities that Claude Code lacks
Now includes SDK-based agent integration for modern Claude Code features
"""
...
from .sdk_integration import SDKIntegrationManager, AgentMode
...
class CCOMOrchestrator:
    """
    Core orchestration engine for CCOM + Claude Code integration v5.0

    Features:
    - Legacy markdown agent support for backward compatibility
    - Modern SDK-based agent integration
    - Intelligent routing between agent types
    - Performance monitoring and migration recommendations
    """
    ...
    def __init__(self):
        ...
        # Initialize SDK Integration Manager
    ...
    def _initialize_sdk_integration(self) -> SDKIntegrationManager:
        ...
        try:
            ...
            sdk_config = self.memory.get("sdk_config", {
    ...
    def invoke_subagent(self, agent_name, context=None, streaming=False):
        """
        Modern CCOM Agent Execution with SDK Integration

        Features:
        - Intelligent routing between SDK and legacy agents
        ...
        try:
            ...
            result = asyncio.run(
                self.sdk_integration.invoke_agent(
            ...
            if hasattr(result, 'success'):
                ...
                if streaming:
                    ...
                    return asyncio.run(handle_streaming())
                else:
                    # Standard result
            ...
            print(f"❌ Agent {agent_name} execution failed: {str(e)}")
            return False

    def invoke_subagent_legacy(self, agent_name):
        """
        Legacy CCOM Agent Execution (Deprecated)

        Maintained for backward compatibility.
        Use invoke_subagent() for new implementations.
        """
        ...
        print(f"🤖 CCOM executing {agent_name} (legacy mode)...")
    ...
    def get_agent_status(self, agent_name=None):
    ...
    def set_agent_mode(self, mode: str):
        ...
        if success:
            ...
            if "sdk_config" not in self.memory:
                self.memory["sdk_config"] = {}
            self.memory["sdk_config"]["agent_mode"] = mode
            self.save_memory()
            print(f"✅ Agent mode set to: {mode}")
        else:
            print(f"❌ Failed to set agent mode: {mode}")
        return success

    async def migrate_to_sdk_mode(self):
        """Migrate to full SDK mode"""
        print("🚀 **CCOM SDK MIGRATION** – Upgrading to modern agent architecture...")
        ...
        if not ready_for_migration:
            print("⚠️ System not ready for full SDK migration")
        ...
        if success:
            # Update memory
            self.memory["sdk_config"] = self.memory.get("sdk_config", {})
            self.memory["sdk_config"]["agent_mode"] = "sdk"
            self.memory["sdk_config"]["migration_date"] = datetime.now().isoformat()
            self.save_memory()

            print("✅ **SDK MIGRATION COMPLETE**")
            print("🎉 CCOM now using modern SDK-based agents for optimal performance")
            return True
        else:
            print("❌ SDK migration failed - remaining in hybrid mode")
            return False
    ...
    def show_sdk_status(self):
        ...
        print(f"🚀 **SDK Agents**: {len(status['sdk_agents'])}")
        print(f"📄 **Legacy Agents**: {len(status['legacy_agents'])}")

        # Show metrics
        metrics = status['metrics']
        if metrics['sdk_executions'] > 0 or metrics['legacy_executions'] > 0:
            print(f"\n📈 **Performance Metrics**:")
            print(f"  SDK Executions: {metrics['sdk_executions']}")
            print(f"  Legacy Executions: {metrics['legacy_executions']}")
            print(f"  SDK Success Rate: {metrics['sdk_success_rate']*100:.1f}%")
            print(f"  Legacy Success Rate: {metrics['legacy_success_rate']*100:.1f}%")
            print(f"  SDK Avg Time: {metrics['average_sdk_time']:.2f}s")
            print(f"  Legacy Avg Time: {metrics['average_legacy_time']:.2f}s")
