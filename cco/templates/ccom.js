...
class CCOM {
  ...
  archiveOldFeatures(days = 30) {
    ...
    try {
      ...
      this.saveMemory();
      console.log(
        `📁 Archived ${toArchive.length} features older than ${days} days to ${path.basename(archiveFile)}`,
      );
  ...
  compactMemory() {
    ...
    for (const [name, data] of Object.entries(this.memory.features)) {
      if (data.description && data.description.length > maxDescLength) {
        ...
        data.description =
          data.description.substring(0, maxDescLength - 3) + "...";
        compacted++;
  ...
  listFeatures(sortBy = "created") {
    ...
    for (const [name, data] of features) {
      ...
      console.log(
        `      Created: ${created.toISOString().split("T")[0]} (${ageStr})`,
      );
    }
    console.log("━".repeat(60));
  }
  ...
  displayMemoryStats() {
    ...
    if (stats.tokens > 20000) {
      console.log(
        "🚨 CRITICAL: Memory usage > 10% of context! Archive immediately!",
      );
    } else if (stats.tokens > 10000) {
      console.log(
        "⚠️ WARNING: Memory usage > 5% of context. Consider archiving.",
      );
