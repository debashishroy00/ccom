...
class AWSDeploymentValidator {
  ...
  validate() {
    console.log(
      "🔍 Validating AWS deployment patterns (ECS, Lambda, S3, API Gateway)...",
    );
  ...
  checkECSService(filePath) {
    try {
      ...
      if (
        ...
        !content.includes("TargetTrackingScaling")
      ) {
        this.addIssue(
          "info",
  ...
  checkServerlessConfig(content, filePath) {
    ...
    if (content.includes("vpc:")) {
      if (
        !content.includes("securityGroupIds") ||
        !content.includes("subnetIds")
      ) {
        this.addIssue(
          "error",
  ...
  checkAPIGatewayConfig(content, filePath) {
    ...
    if (
      !content.includes("authorizer") &&
      !content.includes("authorizationType")
    ) {
      this.addIssue(
        "error",
    ...
    if (
      !content.includes("requestValidator") &&
  ...
  checkS3Configuration(content, filePath) {
    ...
    if (
      !content.includes("VersioningConfiguration") &&
