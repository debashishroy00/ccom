...
class AWSCostValidator {
  ...
  checkBedrockModelUsage(content, filePath) {
    ...
    if (content.includes('claude-3-opus')) {
      ...
      if (content.includes('simple') || content.includes('basic') || content.includes('summary')) {
        this.addIssue('error',
    ...
    if (content.includes('claude') && !content.includes('cost') && !content.includes('track')) {
      this.addIssue('warning',
  ...
  checkTokenLimits(content, filePath) {
    ...
    if (content.includes('maxTokens') && !content.includes('adjust') && !content.includes('dynamic')) {
      this.addIssue('info',
  ...
  checkBatchProcessing(content, filePath) {
    ...
    if (content.includes('embed') && content.includes('for') && !content.includes('batch')) {
      this.addIssue('error',
  ...
  checkCachingStrategies(content, filePath) {
    ...
    if (content.includes('cache') && !content.includes('ttl') && !content.includes('expir')) {
      this.addIssue('warning',
  ...
  validateLambdaCosts() {
    const lambdaFiles = this.findFiles(['*lambda*.js', '*lambda*.py', 'serverless.yml']);
    ...
    lambdaFiles.forEach(file => {
      try {
        ...
        this.addIssue('error', `Cannot read Lambda file: ${file}`, error.message);
      }
    });
  }
  ...
  validateECSCosts() {
    const ecsFiles = this.findFiles(['*ecs*.json', '*task-def*.json', '*fargate*.json']);
  ...
  checkECSResourceAllocation(config, filePath) {
    ...
    if (!JSON.stringify(config).includes('autoScaling') &&
  ...
  generateReport() {
    ...
    if (this.costEstimates.total) {
      ...
      console.log(`  Bedrock: $${this.costEstimates.bedrock.monthly['claude-3-sonnet'] + this.costEstimates.bedrock.monthly['titan-embed']}`);
