...
class MongoDBVectorValidator {
  ...
  validateConnection() {
    const configFiles = this.findFiles(['*.config.js', '*.config.ts', '.env', '*.yml']);
  ...
  checkConnectionConfig(content, filePath) {
    ...
    if (content.includes('mongodb+srv://') || content.includes('atlas')) {
      ...
      if (!content.includes('w=majority') && !content.includes('writeConcern')) {
        this.addIssue('warning',
  ...
  validateVectorSearchIndexes() {
    const indexFiles = this.findFiles(['*index*.js', '*index*.json', '*schema*.js']);
  ...
  checkVectorIndexConfig(content, filePath) {
    ...
    if (dimMatch) {
      ...
      const validDimensions = Object.values(ATLAS_INDEX_CONFIG.vectorDimensions);
    ...
    files.forEach(file => {
      try {
        ...
        if (content.includes('sensitive') || content.includes('personal')) {
          if (!content.includes('ClientEncryption') && !content.includes('CSFLE')) {
            this.addIssue('info',
    ...
    queryFiles.forEach(file => {
      try {
        ...
        if (content.includes('insertOne') || content.includes('updateOne')) {
          if (!content.includes('bulkWrite') && !content.includes('insertMany')) {
            this.addIssue('info',
        ...
        if (content.includes('watch(')) {
          if (!content.includes('resumeAfter') && !content.includes('startAfter')) {
            this.addIssue('warning',
