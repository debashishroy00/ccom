...
const HYBRID_PATTERNS = {
  ...
  fusion: {
    ...
    validations: [
      {
        check: (content) =>
          content.includes("normalize") || content.includes("normalization"),
  ...
  reranking: {
    ...
    validations: [
      {
        check: (content) =>
          content.includes("top_k") || content.includes("limit"),
        message: "Reranking should limit input size for performance",
      },
      {
        check: (content) =>
          content.includes("batch") || content.includes("concurrent"),
  ...
  vectorKeyword: {
    ...
    validations: [
      ...
      {
        check: (content) =>
          content.includes("normalize") || content.includes("min_max"),
  ...
  graphRag: {
    ...
    validations: [
      {
        check: (content) =>
          content.includes("hop") || content.includes("depth"),
        message:
          "Graph traversal should limit hop count to prevent exponential expansion",
      },
      {
        check: (content) =>
          content.includes("entity") && content.includes("filter"),
...
class HybridRAGValidator {
  ...
  validateProject() {
    console.log(
      "🔍 CCOM HYBRID RAG VALIDATION – Analyzing hybrid retrieval patterns...",
    );
  ...
  checkPerformanceSettings(content, filePath) {
    ...
    performanceChecks.forEach(({ exists, message }) => {
      if (!exists) {
        this.addIssue(
          "info",
          `Performance optimization in ${filePath}`,
          message,
        );
      }
    });
  ...
  validateFusionInFile(filePath) {
    try {
      ...
      this.addIssue(
        "error",
        `Cannot read fusion file: ${filePath}`,
        error.message,
      );
    }
  }
  ...
  validateFusionImplementation(content, filePath, methodName) {
    ...
    validations.forEach(({ check, message }) => {
      if (!check) {
        this.addIssue(
          "warning",
          `${methodName} implementation in ${filePath}`,
          message,
        );
      }
    });
  ...
  checkWeightValidation(content, filePath) {
    ...
    if (
      ...
      (content.includes("fusion") || content.includes("combine"))
    ) {
      const hasValidation = [
  ...
  checkNormalizationMethods(content, filePath) {
    ...
    if (content.includes("normalize") || content.includes("normalization")) {
      ...
      const hasMethod = normalizationMethods.some(({ pattern }) =>
  ...
  validateRerankingInFile(filePath) {
    try {
      ...
      this.addIssue(
        "error",
        `Cannot read reranking file: ${filePath}`,
        error.message,
      );
    }
  }
  ...
  validateGraphRAGInFile(filePath) {
    try {
      ...
      this.addIssue(
        "error",
        `Cannot read graph RAG file: ${filePath}`,
        error.message,
      );
    }
  }
  ...
  checkGraphScoring(content, filePath) {
    ...
    if (content.includes("graph") && content.includes("score")) {
      const hasGraphScoring =
        content.includes("pagerank") ||
  ...
  checkPerformanceLimits(filePath) {
    try {
      ...
      this.addIssue(
        "error",
        `Cannot read performance config: ${filePath}`,
        error.message,
      );
    }
  }
