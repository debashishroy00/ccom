...
class AngularValidator {
  ...
  validateAngularProject() {
    ...
    if (!fs.existsSync(angularJson) && !fs.existsSync(packageJson)) {
      this.addIssue('info', 'Not an Angular project', 'Angular validator skipped');
      return;
    }
    ...
    if (fs.existsSync(packageJson)) {
      try {
        ...
        if (!angularCore) {
          this.addIssue('info', 'Angular not detected in dependencies', 'Skipping Angular validation');
          return;
        }
        ...
        if (!rxjs) {
          this.addIssue('warning', 'RxJS not found in dependencies', 'RxJS required for Angular reactive patterns');
        }
  ...
  checkSubscriptionPatterns(content, filePath) {
    const subscribeMatches = content.match(ANGULAR_PATTERNS.rxjs.subscriptions[0]);
    const unsubscribeMatches = content.match(/\.unsubscribe|takeUntil|takeWhile|first\(\)|take\(1\)/g);

    if (subscribeMatches) {
      const subscribeCount = subscribeMatches.length;
      const unsubscribeCount = unsubscribeMatches ? unsubscribeMatches.length : 0;
  ...
  checkMemoryLeakPatterns(content, filePath) {
    ...
    MEMORY_LEAK_PATTERNS.subscriptions.forEach(pattern => {
      if (pattern.test(content)) {
        if (!content.includes('clearInterval') && !content.includes('takeUntil')) {
          this.addIssue('error',
  ...
  checkUnsubscribePatterns(content, filePath) {
    ...
    if (content.includes('subscribe(')) {
      ...
      if (content.includes('takeUntil')) {
        if (!content.includes('destroy$') && !content.includes('unsubscribe$')) {
          this.addIssue('info',
      ...
      if (!content.includes('subscription') && !content.includes('takeUntil') && !content.includes('| async')) {
        this.addIssue('warning',
  ...
  validateChangeDetection() {
    ...
    componentFiles.forEach(file => {
      try {
        ...
        this.addIssue('error', `Cannot read component file: ${file}`, error.message);
      }
    });
  }
  ...
  checkTemplatePatterns(templateFile) {
    try {
      ...
      if (content.includes('()') && (content.includes('.length') || content.includes('function'))) {
        this.addIssue('error',
      ...
      this.addIssue('error', `Cannot read template file: ${templateFile}`, error.message);
    }
  }
  ...
  validatePerformancePatterns() {
    ...
    files.forEach(file => {
      try {
        ...
        if (content.includes('ngFor') && !content.includes('cdk-virtual-scroll')) {
          this.addIssue('info',
  ...
  validateRAGSpecificPatterns() {
    ...
    serviceFiles.forEach(file => {
      try {
        ...
        this.addIssue('error', `Cannot read service file: ${file}`, error.message);
      }
    });
  }
  ...
  checkErrorHandling(content, filePath) {
    ...
    if (content.includes('error') && !content.includes('user') && !content.includes('message')) {
      this.addIssue('info',
