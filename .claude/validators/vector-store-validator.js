...
class VectorStoreValidator {
  ...
  validateProject() {
    console.log(
      "🔍 CCOM VECTOR VALIDATION – Analyzing vector store patterns...",
    );
  ...
  validateConfigurations() {
    ...
    const configPatterns = [
      ...
      "qdrant.config.js",
      "vector.config.js",
      "embeddings.config.js",
      "mongodb-atlas.config.js",
      "atlas-search.js",
      "mongodb-vector.js",
      "atlas-vector-search.js",
    ];
  ...
  checkSimilarityQueries(content, filePath) {
    ...
    const queryPatterns = [/\.query\(/g, /\.search\(/g, /\.similarity\(/g];
  ...
  validatePerformanceLimits() {
    ...
    if (fs.existsSync(packagePath)) {
      ...
      const hasMemoryLimit = Object.values(scripts).some(
        (script) =>
          script.includes("--max-old-space-size") ||
          script.includes("--max-heap-size"),
      );
  ...
  checkMetadataInjection(content, filePath) {
    ...
    if (
      (content.includes("metadata") && content.includes("${")) ||
      content.includes("`${")
    ) {
      this.addIssue(
        "warning",
