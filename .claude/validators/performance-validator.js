...
class PerformanceValidator {
  ...
  validateMonitoring() {
    const files = this.findFiles(['*.js', '*.ts', '*.py', '*.yml', '*.yaml', '*.json']);
    ...
    files.forEach(file => {
      try {
        ...
        if (PERFORMANCE_PATTERNS.monitoring.cloudwatch.some(pattern => pattern.test(content))) {
          hasCloudWatch = true;
        }

        // Check for X-Ray tracing
        if (PERFORMANCE_PATTERNS.monitoring.xray.some(pattern => pattern.test(content))) {
          hasXRay = true;
        }

        // Check for custom performance metrics
        if (PERFORMANCE_PATTERNS.monitoring.custom.some(pattern => pattern.test(content))) {
  ...
  checkCustomMetrics(content, filePath) {
    ...
    const ragMetrics = ['embedding_time', 'retrieval_time', 'generation_time', 'total_latency'];
    const foundMetrics = ragMetrics.filter(metric => content.includes(metric));
    ...
    if (!content.includes('p95') && !content.includes('p99') && !content.includes('percentile')) {
      this.addIssue('info',
  ...
  validateCaching() {
    ...
    files.forEach(file => {
      try {
        ...
        if (PERFORMANCE_PATTERNS.caching.redis.some(pattern => pattern.test(content))) {
          hasRedis = true;
          this.checkRedisCaching(content, file);
        }

        // Check for memory caching
        if (PERFORMANCE_PATTERNS.caching.memory.some(pattern => pattern.test(content))) {
          hasMemoryCache = true;
          this.checkMemoryCaching(content, file);
        }

        // Check for CDN usage
        if (PERFORMANCE_PATTERNS.caching.cdn.some(pattern => pattern.test(content))) {
          hasCDN = true;
        }
  ...
  checkRedisCaching(content, filePath) {
    ...
    if (!content.includes('expire') && !content.includes('ttl') && !content.includes('EX')) {
      this.addIssue('error',
  ...
  checkMemoryCaching(content, filePath) {
    ...
    if (!content.includes('maxSize') && !content.includes('max.*size') && !content.includes('limit')) {
      this.addIssue('warning',
    ...
    if (content.includes('cache') && !content.includes('lru') && !content.includes('LRU')) {
      this.addIssue('info',
  ...
  checkEmbeddingPerformance(content, filePath) {
    if (PERFORMANCE_PATTERNS.rag.embedding.some(pattern => pattern.test(content)) ||
        content.includes('embedding') || content.includes('embed')) {

      // Check for batch processing
      if (!content.includes('batch') && content.includes('for') && content.includes('embed')) {
        this.addIssue('error',
  ...
  checkRetrievalPerformance(content, filePath) {
    if (PERFORMANCE_PATTERNS.rag.retrieval.some(pattern => pattern.test(content)) ||
        content.includes('search') || content.includes('retriev')) {
      ...
      if (!content.includes('limit') && !content.includes('top_k') && !content.includes('topK')) {
        this.addIssue('error',
      ...
      if (content.includes('multiple') && !content.includes('parallel') && !content.includes('concurrent')) {
        this.addIssue('info',
  ...
  checkGenerationPerformance(content, filePath) {
    if (PERFORMANCE_PATTERNS.rag.generation.some(pattern => pattern.test(content)) ||
        content.includes('generate') || content.includes('llm') || content.includes('claude')) {
  ...
  checkLambdaOptimization(content, filePath) {
    if (content.includes('lambda') || content.includes('Lambda')) {
      ...
      if (!OPTIMIZATION_PATTERNS.lambda.coldStart.some(pattern => pattern.test(content))) {
        this.addIssue('info',
      ...
      if (!OPTIMIZATION_PATTERNS.lambda.memory.some(pattern => pattern.test(content))) {
        this.addIssue('info',
  ...
  checkDatabaseOptimization(content, filePath) {
    if (content.includes('database') || content.includes('db') || content.includes('mongo') || content.includes('sql')) {
      // Check for connection pooling
      if (!OPTIMIZATION_PATTERNS.database.connection.some(pattern => pattern.test(content))) {
        this.addIssue('warning',
      ...
      if (!OPTIMIZATION_PATTERNS.database.query.some(pattern => pattern.test(content))) {
        this.addIssue('info',
  ...
  checkFrontendOptimization(content, filePath) {
    if (filePath.includes('angular') || filePath.includes('component') || filePath.includes('service')) {
      // Check for bundling optimizations
      if (!OPTIMIZATION_PATTERNS.frontend.bundling.some(pattern => pattern.test(content))) {
        this.addIssue('info',
      ...
      if (!OPTIMIZATION_PATTERNS.frontend.caching.some(pattern => pattern.test(content))) {
        this.addIssue('info',
  ...
  validateLatencyPatterns() {
    ...
    files.forEach(file => {
      try {
        ...
        if (content.includes('performance') || content.includes('time') || content.includes('latency')) {
  ...
  checkLatencyMeasurement(content, filePath) {
    ...
    const timingMethods = ['performance.now', 'Date.now', 'process.hrtime', 'time.time'];
    const hasTimingMethod = timingMethods.some(method => content.includes(method));
    ...
    if (!content.includes('threshold') && !content.includes('sla') && !content.includes('target')) {
      this.addIssue('warning',
  ...
  validateThroughputOptimization() {
    ...
    files.forEach(file => {
      try {
        ...
        if (content.includes('process') && !content.includes('parallel') && !content.includes('concurrent')) {
          this.addIssue('info',
        ...
        if (content.includes('process') && !content.includes('queue') && !content.includes('Queue')) {
          this.addIssue('info',
