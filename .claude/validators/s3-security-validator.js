...
class S3SecurityValidator {
  ...
  checkPresignedUrlGeneration(content, filePath) {
    const hasPresignedUrls = S3_SECURITY_PATTERNS.presignedUrls.generation.some(pattern =>
      pattern.test(content)
    );

    if (hasPresignedUrls) {
      // Check for expiration settings
      if (!S3_SECURITY_PATTERNS.presignedUrls.security.some(pattern => pattern.test(content))) {
      ...
      if (!content.includes('ContentType') && !content.includes('content-type')) {
        this.addIssue('error',
      ...
      if (!content.includes('ContentLength') && !content.includes('content-length')) {
  ...
  checkPresignedUrlSecurity(content, filePath) {
    ...
    if (content.includes('createPresignedPost') || content.includes('getSignedUrl')) {
      ...
      if (!content.includes('key.*prefix') && !content.includes('starts-with')) {
        this.addIssue('info',
  ...
  checkMultipartImplementation(content, filePath) {
    const hasMultipart = S3_SECURITY_PATTERNS.multipart.patterns.some(pattern =>
      pattern.test(content)
    );

    if (hasMultipart) {
      // Check for complete implementation
      const requiredMethods = ['createMultipartUpload', 'uploadPart', 'completeMultipartUpload'];
  ...
  checkMultipartCleanup(content, filePath) {
    if (content.includes('multipart')) {
      ...
      if (!S3_SECURITY_PATTERNS.multipart.security.some(pattern => pattern.test(content))) {
        this.addIssue('warning',
  ...
  validateCORSConfiguration() {
    ...
    configFiles.forEach(file => {
      try {
        ...
        if (S3_SECURITY_PATTERNS.cors.patterns.some(pattern => pattern.test(content))) {
  ...
  checkCORSSecurity(content, filePath) {
    ...
    if (content.includes('AllowedHeaders') && !content.includes('Content-Type')) {
      this.addIssue('warning',
  ...
  validateEncryption() {
    const files = this.findFiles(['*.json', '*.yml', '*.yaml', '*.tf', '*.js', '*.ts', '*.py']);
    ...
    files.forEach(file => {
      try {
        ...
        if (content.includes('s3') || content.includes('S3')) {
          const hasEncryptionConfig = S3_SECURITY_PATTERNS.encryption.patterns.some(pattern =>
            pattern.test(content)
          );
  ...
  checkEncryptionConfiguration(content, filePath) {
    ...
    if (!S3_SECURITY_PATTERNS.encryption.required.some(pattern => pattern.test(content))) {
      this.addIssue('error',
    ...
    if (!content.includes('BucketEncryption') && !content.includes('default.*encryption')) {
      this.addIssue('warning',
  ...
  validateAccessPolicies() {
    const policyFiles = this.findFiles(['*policy*.json', '*bucket*.json', '*.tf']);
  ...
  checkAccessPolicySecurity(content, filePath) {
    ...
    if (content.includes('Resource') && content.includes('*') && !content.includes('arn:aws:s3:::')) {
      this.addIssue('warning',
